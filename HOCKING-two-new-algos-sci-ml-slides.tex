...
\begin{frame}
  \frametitle{Brief bio, Sherbrooke}
  \begin{itemize}
  \item Bachelor, Berkeley Stats 2006; PHD, Paris 2012.
  \item CS Prof Northern Arizona University, 2018--2024.
  \item CS Prof Université de Sherbrooke, 2024--present.
  \item Population City=200,000 University=30,000 (est. 1954).
  \item Teaching in French, bilingual research.
  \end{itemize}
  \includegraphics[width=\textwidth]{universite-sherbrooke-udes-uds-pavillon-logo.png}
\end{frame}

\begin{frame}
  \frametitle{LASSO lab --- \href{http://lassolab.org}{lassolab.org} }
  \begin{itemize}
  \item Name inspired by influential paper, Tibshirani (JRSSB, 1996).
  \item Learning Algorithms, Statistical Software, Optimization.
  \item Fundamental new methods useful in different applications: new neural networks, loss functions, optimization algorithms, data visualization {\small\url{https://animint.github.io/gallery}}, ...
  \item Impactful collaborative research: carbon emissions estimation from satellite data, fish spawning localization from sonar data, interpretable models for autism, genomic data segmentation, global soil carbon modeling (Nature papers, 2023--2024), ...
  \item Contributions to widely-used software: R, pandas, data.table, xgboost, mlr3, torch, ...
  \item 30+ research students mentored at NAU and Sherbrooke.
  \item Mentorship for 30+ online coding internships, R in Google Summer of Code (2013--present).
  \item Hiring PHD students to start next year!
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \includegraphics[width=\textwidth]{MNIST_FashionMNIST_error_glmnet_featureless_mean_SD_zoom_useful.png}
  \begin{itemize}
  ...
  \item \textbf{Other cv\_glmnet} has greater test error than
    \textbf{featureless}.
  \item The patterns in the two subsets are too different for \textbf{Other cv\_glmnet} to learn anything useful for prediction. (expected)
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \includegraphics[width=\textwidth]{MNIST_EMNIST_error_glmnet_featureless_mean_SD_zoom_useful.png}
  \begin{itemize}
...
\begin{frame}
  ...
  \includegraphics[width=\textwidth]{MNIST_EMNIST_rot_error_glmnet_featureless_mean_SD_zoom_useful.png} 
  \begin{itemize}
...
\begin{frame}
  ...
  \end{itemize}
  %  --- slightly beneficial to combine
  % subsets when training with this algorithm (but not with linear
  % model).
\end{frame}
...
\begin{frame}
  \frametitle{Benchmark data with pre-defined train/test subsets}
  \includegraphics[width=\textwidth]{data_Classif_batchmark_registry_glmnet_featureless_mean_sd_useful_all_test}
\end{frame}

\begin{frame}
  \frametitle{Benchmark data with pre-defined train/test subsets}
  \includegraphics[width=\textwidth]{data_Classif_batchmark_registry_glmnet_featureless_mean_sd_beneficial_all_test}
\end{frame}

\begin{frame}
  \frametitle{Benchmark data with pre-defined train/test subsets}
  \includegraphics[width=\textwidth]{data_Classif_batchmark_registry_glmnet_featureless_mean_sd_beneficial_all_train}
\end{frame}

\begin{frame}
  \frametitle{Benchmark data with pre-defined train/test subsets}
  \includegraphics[width=\textwidth]{data_Classif_batchmark_registry_glmnet_featureless_mean_sd_beneficial_other_train}
\end{frame}

\begin{frame}
  \frametitle{Benchmark data with pre-defined train/test subsets}
  \includegraphics[width=\textwidth]{data_Classif_batchmark_registry_glmnet_featureless_mean_sd_beneficial_other_test}
\end{frame}
...
\begin{frame} \frametitle{ROC curve motivation 1: comparing algorithms}
  How to compare algorithms with different false positive rates?
  \begin{itemize}
   \item Ex: two algorithms with 50 positive and 50 negative labels.
   \begin{itemize}
    \item algo A 5 incorrectly predicted positive labels (FPR=0).
    \item algo B 5 incorrectly predicted negative labels (FPR=10\%).
    \item both have label error rate of 5\%.
    \item which is better?
   \end{itemize}
 \item ROC curves show TPR=1-FNR for all possible FPR values, so are fair to algorithms with different false positive rates.
   \begin{itemize}
   % \item TPR = True Positive Rate
   %   (correctly predicted positive labels)
   %    = 1-False Negative Rate
   % \item FPR = False Positive Rate (incorrectly predicted negative labels)
   \item Can compare TPR using a fixed relevant FPR, ex: compare TPR of algos A and B, when both algos have FPR=5\%.
   \item Or average over all FPR: Area Under ROC Curve (AUC, larger values mean more accurate).
   \end{itemize}
  \end{itemize}
\end{frame}

\begin{frame} \frametitle{ROC curve motivation 2: class imbalance}
  \begin{itemize}
  \item How do we know if our learning algorithm is working?
  \item Ex: neural network predicts 95\% labels accurately in test set.
  \item Is this good or bad? It depends.
  \item 3\% autism, 97\% not: 95\% accuracy is worse than featureless
    baseline! (which always predicts most frequent class, 97\% accuracy)
  \item 50\% positive, 50\% negative (balanced labels): 95\% accuracy
    is definitely better than featureless baseline (50\% accuracy).
  \item featureless baseline always has
    Area Under ROC Curve (AUC) of 0.5, so AUC$>0.5$ means the algorithm has learned something non-trivial (regardless of class balance).
  \end{itemize}
\end{frame}
...
\begin{itemize}
\item Imbalanced binary classification: 10\% negative, 90\% positive.
...
\begin{frame}
  ...
  \item Proposed AUM loss can be used in gradient descent instead of logistic/binary cross-entropy loss. Python torch code!
  \item R: \texttt{torch::nn\_aum\_loss} since June 2025, \url{https://github.com/tdhock/mlr3torchAUM}
